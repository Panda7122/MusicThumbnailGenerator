...
def main():
    ...
    for index, row in dataset.iterrows():
        ...
        # Save midi_embeding to ./input/midi with name {artist}-{title}
        midi_save_dir = './input/midi'
        os.makedirs(midi_save_dir, exist_ok=True)
        midi_save_path = os.path.join(midi_save_dir, f"{artist}-{title}.pt")
        torch.save(midi_embeding.cpu(), midi_save_path)
        print(f"MIDI embedding saved to {midi_save_path}")
        text_save_dir = './input/text'
        os.makedirs(text_save_dir, exist_ok=True)
        text_save_path = os.path.join(text_save_dir, f"{artist}-{title}.pt")
        torch.save(text_embeddings.cpu(), text_save_path)
        print(f"Text embedding saved to {midi_save_path}")
